...
public class ElasticPoolUtil {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ElasticPoolUtil.class);
    ...
    public ElasticPoolUtil(String host, String username, String password) {
     ...
     * @param type-ES的type
     * @param source-更新的内容,                                          key为id，value为source
     * @param scriptFile-放在                                          elasticsearch home 目录下的 config/script 目录下的groovy脚本, 为了安全,不支持在请求中带上脚本 之所以是 groovy 脚本,因为 groovy 是 2.x 和 5.x 都支持的
     * @param lang-脚本的语言类型,                                          支持 groovy, painless
     * @param upsert-文档不存在时插入,其实控制粒度是对于每一个文档的,但是这里为了方便输入,粒度为同一次批量的文档
     * @return
     */
    public BulkResponse bulkUpdateByScript(String index, String type, Map<String, JsonObject> source, String
            scriptFile, String lang, boolean upsert) {
        String idFormat = "{\"update\": {\"_id\": \"%s\"}}\n";
        if (null == index || null == type) {
            JsonObject insertError = new JsonObject();
            insertError.addProperty("msg", "bulk update must specify index and type");
     ...
     * @return
     */
    public String updataByQuery(String index, String type, boolean proceedConflicts, int scrollSize, int slices,
                                boolean wait, JsonObject query) {
        return xxByQuery("update", index, type, proceedConflicts, scrollSize, slices, wait, query);
    }

    public String updataByQuery(String index, String type, boolean proceedConflicts, int scrollSize, int slices,
                                boolean wait, SearchRequest searchRequest) {
        try {
     ...
     * @return
     */
    public String deleteByQuery(String index, String type, boolean proceedConflicts, int scrollSize, int slices,
                                boolean wait, JsonObject query) {
