...
public class ElasticUtil {
	...
	/**
	 * {a,b} 用中括号和逗号进行包装, 用于聚合返回的结果中带上多个源字段，或者搜索多个字段 ["a", "b"]
	 *
	 * @return 返回结果
	 */
	...
	public BulkResponse bulkInsert(JsonArray array) {
		...
		for (JsonElement e : array) {
			...
			if (!e.getAsJsonObject().has("_type")) {
				...
				entity.append(
						String.format(indexWithType, e.getAsJsonObject().get("_index").getAsString(), e.getAsJsonObject().get("_type").getAsString(), e.getAsJsonObject().get("_id").getAsString()));
			}
	...
	public BulkResponse bulkUpdate(JsonArray array, boolean upsert) {
		...
		for (JsonElement e : array) {
			...
			if (!e.getAsJsonObject().has("_type")) {
				...
				entity.append(
						String.format(updateWithType, e.getAsJsonObject().get("_index").getAsString(), e.getAsJsonObject().get("_type").getAsString(), e.getAsJsonObject().get("_id").getAsString()));
			}
			e.getAsJsonObject().remove("_index");
			e.getAsJsonObject().remove("_type");
			e.getAsJsonObject().remove("_id");
	...
	public BulkResponse bulkDelete(JsonArray array) {
		...
		for (JsonElement e : array) {
			...
			if (!e.getAsJsonObject().has("_type")) {
				...
				entity.append(
						String.format(deleteWithType, e.getAsJsonObject().get("_index").getAsString(), e.getAsJsonObject().get("_type").getAsString(), e.getAsJsonObject().get("_id").getAsString()));
			}
			e.getAsJsonObject().remove("_index");
			e.getAsJsonObject().remove("_type");
			e.getAsJsonObject().remove("_id");
