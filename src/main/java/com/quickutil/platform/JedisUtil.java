...
public class JedisUtil {
     ...
     * @param jedis-jedis配置
     */
    public static void addJedisPool(Properties jedis) {
        addJedisPool(jedis, null);
    }
     ...
     * @param pool-pool配置
     */
    public static void addJedisPool(Properties jedis, Properties pool) {
     ...
     * @param key-key名
     */
    public static void deleteKey(String dbName, String key) {
        JedisPool pool = JedisPoolMap.get(dbName);
        Jedis jedis = pool.getResource();
        jedis.del(key);
    }
     ...
     * @param dbName-数据库名称
     */
    public static void flushData(String dbName) {
        JedisPool pool = JedisPoolMap.get(dbName);
        Jedis jedis = pool.getResource();
        jedis.flushDB();
    }
     ...
     * @param seconds-有效时间
     */
    public static boolean setExpire(String dbName, String key, int seconds) {
        JedisPool pool = JedisPoolMap.get(dbName);
        Jedis jedis = pool.getResource();
        try {
     ...
     * @param value-数据内容
     */
    public static boolean setString(String dbName, String key, String value) {
        JedisPool pool = JedisPoolMap.get(dbName);
        Jedis jedis = pool.getResource();
        try {
            jedis.set(key, value);
     ...
     * @param list-数据内容
     */
    public static boolean pushQueue(String dbName, String key, List<String> list) {
        JedisPool pool = JedisPoolMap.get(dbName);
        Jedis jedis = pool.getResource();
        try {
     ...
     * @param list-数据内容
     */
    public static boolean backtoQueue(String dbName, String key, List<String> list) {
        JedisPool pool = JedisPoolMap.get(dbName);
        Jedis jedis = pool.getResource();
        try {
     ...
     * @return
     */
    public static boolean setHash(String dbName, String key, Map<String, String> hash) {
        JedisPool pool = JedisPoolMap.get(dbName);
        Jedis jedis = pool.getResource();
        try {
    ...
    public static List<String> getHashX(String dbName, List<String> keyList, List<String> fieldList) {
        ...
        try {
            ...
            String[] params = paramList.toArray(new String[paramList.size()]);
            List<String> list = (List<String>) jedis.eval(getHashLua, 0, params);
            return list;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
     ...
     */
    public static void setHincrby(String dbName, List<String> keyList, List<String> fieldList, List<Long> countList) {
        JedisPool pool = JedisPoolMap.get(dbName);
        Jedis jedis = pool.getResource();
        try {
            List<String> paramList = new LinkedList<String>();
